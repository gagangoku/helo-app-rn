...
import {
    ...
    FORCE_UPDATE_AFTER_APP_VERSION,
    GOLD_CODE_LENGTH,
...
import {notifyPushyMembers} from "./pushy";
import {getLastReadIdx} from "./FirebaseDataListener";
import {convertXLSXToHeloAndPostInGroup} from "./XLSXConverter";
...
app.post('/sendNotificationToMembers', async (req, res) => {
    ...
    notifyPushyMembers(req.body);
    res.status(201).json({});
});

app.get('/v1/app/what-to-do', async (req, res) => {
    const fullUrl = req.protocol + '://' + req.get('host') + req.originalUrl;
    const appVersion = getUrlParam('appVersion', fullUrl);
    const forceUpdate = semver.lt(appVersion, FORCE_UPDATE_AFTER_APP_VERSION);

    res.status(200).json({ forceUpdate, message: 'Your app is too old, please update', appUpdateUrl: APP_UPDATE_URL });
});

app.get('/firestore/last-read-idx', async (req, res) => {
    const fullUrl = req.protocol + '://' + req.get('host') + req.originalUrl;
    const roleId = getUrlParam('roleId', fullUrl);

    const obj = getLastReadIdx(roleId);
    res.status(200).json(obj);
});

// Convert .xlsx file into sheet
app.post('/xlsx/convert', async (req, res) => {
    const { fileUrl, groupId, sheetName, sender, collection } = req.body;
    cnsole.info('/xlsx/convert: ', { fileUrl, groupId, sheetName, sender });
    if (!fileUrl || !groupId || !sender) {
        res.status(500).json({ success: false, reason: 'Bad request' });
        return;
    }
    await convertXLSXToHeloAndPostInGroup({ fileUrl, groupId, sheetName, sender, collection: collection || FIREBASE_GROUPS_DB_NAME });
