...
export const getOcrJson = async (file) => {
    const url = format('/static/ocr/{}', file);
    return await new Promise((responseCb, errorCb) => _fetchUrl(url, null, true, responseCb, errorCb, 'GET'));
};

const _fetchUrlWithTimeout = async (url, timeout, body, consumeJson, method, headers={}) => {
    try {
        const obj = { timedOut: false };
        ...
        return obj.timedOut ? null : await (consumeJson ? response.json() : response.text());
    } catch (e) {
        cnsole.warn('Exception in _fetchUrlWithTimeout: ', url, e);
        return null;
    }
};
...
const _fetchUrl = function(url, body, consumeJson, responseCb, errorCb, method, headers={}) {
    ...
    errorCb = errorCb || ((x) => {});
    method = method || 'GET';
    cnsole.info('Sending ', method, ' request to: ' + url);
    ...
    fetch(url, {
        ...
        if (response.status >= 400) {
            cnsole.warn('Bad response: ', response);
            errorCb(response);
        } else {
            ...
            out.then((obj) => {
                ...
                cnsole.warn('exception in url: ', url, ex);
                errorCb(ex);
            });
        }
    }).catch((ex) => {
        cnsole.warn('Exception in url fetch: ', url, ex);
        errorCb(ex);
    });
};
...
export const getLocationFromIPAddress = async (deviceID) => {
    ...
    cnsole.info('getLocationFromIPAddress sending request');
    const data = await _fetchUrlWithTimeout(url, 2000, { 'considerIp': true }, true, 'POST', {'Content-Type': 'application/json'});
    cnsole.info('getLocationFromIPAddress took: ', new Date().getTime() - startTimeMs);
...
export const getPersonNamesByRoleId = async (roleIds) => {
    if (roleIds.filter(x => !x).length > 0) {
        cnsole.error('Bad roleIds: ', roleIds);
    }
    const customerIds = roleIds.filter(x => x && x.startsWith('cust')).map(x => parseInt(x.split(':')[1]));
    const supplyIds = roleIds.filter(x => x && x.startsWith('supply')).map(x => parseInt(x.split(':')[1]));
    const visitorIds = roleIds.filter(x => x && x.startsWith('visitor')).map(x => parseInt(x.split(':')[1]));
    ...
    if (visitorIds.length > 0) {
        ...
        visitors.forEach(x => {
            const {id, name, deviceID, photo, designation} = x;
            map['visitor:' + id] = { person: {id, name, deviceID, image: photo, photo, designation} };
        });
    }
...
export const sendMwebNotificationToMembers = async (memberRoleIds, sender, message, iconUrl, clickUrl) => {
    const url = format('{}/sendNotificationToMembers', MWEB_URL);
    const obj = { memberRoleIds, sender, message, iconUrl, clickUrl };
    const h = {[X_AUTH_HEADER]: X_AUTH_TOKEN, 'Content-Type': 'application/json'};
    return await new Promise((responseCb, errorCb) => _fetchUrl(url, obj, false, responseCb, errorCb, 'POST', h));
};
export const sendServerFirebaseNotificationToTopic = async ({ topics, title, body, image, action }) => {
    const obj = { topics, title, body, image, action };
    const url = format('{}/v1/firebase/notifyTopics', API_URL);
    const h = {[X_AUTH_HEADER]: X_AUTH_TOKEN, 'Content-Type': 'application/json'};
    return await new Promise((responseCb, errorCb) => _fetchUrl(url, obj, false, responseCb, errorCb, 'POST', h));
};
...
export const whatToDoWithAppVersion = async () => {
    const url = format(MWEB_URL + WHAT_TO_DO_WITH_APP_API + '?appVersion={}', APP_VERSION);
    const h = {[X_AUTH_HEADER]: X_AUTH_TOKEN};
    return await _fetchUrlWithTimeout(url, 1000, null, true, 'GET', h);
};
