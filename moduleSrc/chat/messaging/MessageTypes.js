...
import {
    actionButton,
    findPhonesAndLinksInText,
    formatPhonesAndLinksForDisplayHtml,
    getCircularImage,
    getKeysWhereValueIs,
    hashCode,
...
import {goToChatFn, playVideoFullscreen, showPersonDetailsPageOnclickFn} from "../../platform/Navigators";
import {Spreadsheet} from "../../platform/Spreadsheet";
import {LongpressWrapper} from "../../platform/LongpressWrapper";
import {getNavigationObject} from "../../router/NavigationRef";
import {TaskList} from "../../platform/TaskList";
...
class TextMessage extends React.PureComponent {
    ...
    render () {
        ...
        cnsole.info('TextMessage render: ', idx);
        let { text } = message;
        ...
        const { allLinks } = findPhonesAndLinksInText(text);
        const tt = formatPhonesAndLinksForDisplayHtml({ text, allLinks, me: me.sender, messageSender: message.sender });
        const html = renderHtmlText(tt, { ...styleObj, whiteSpace: 'pre-wrap', wordWrap: 'break-word' });

        return (
            <LongpressWrapper key={idx} {...this.props}>
                <MessageShell {...this.props} message={{...message, type: OUTPUT_TEXT}}>
                    <View style={{}}>{html}</View>
                </MessageShell>
            </LongpressWrapper>
        );
    }
...
class ProgressiveModule extends React.PureComponent {
    openProgressiveModule = () => {
        ...
        playVideoFullscreen({ data: { collection, groupId, idx, me, videoUrl }, navigation });
    };
    openLeaderboard = () => {
        const { message, idx, me, groupId, collection } = this.props;
        const { text } = message;
        const leaderboardFn = this.props.leaderboardFn || (() => {});
        leaderboardFn({ idx, message, me, groupId, collection, moduleName: text });
    };

    render () {
        cnsole.info('ProgressiveModule render');
        const { message, idx, me, groupId, collection } = this.props;
        ...
        const leaderBoardIcon = (
            <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <TouchableAnim onPress={this.openLeaderboard}>
            ...
            <LongpressWrapper key={idx} {...this.props}>
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: sender === me.sender ? 'flex-end' : 'flex-start' }}>
                    ...
                    <MessageShell key={idx} {...this.props}>
                        <View style={{ width: 200, marginLeft: 3, marginRight: 3, marginTop: 3 }}>
                            <TouchableAnim onPress={this.openProgressiveModule} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
...
class ImageMessage extends React.PureComponent {
    ...
    render () {
        ...
        cnsole.info('ImageMessage render: ', idx);
...
class AudioMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('AudioMessage render');
...
class VideoMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('VideoMessage render');
        const { message, idx, me, otherGuy, onNewMsgFn, mode } = this.props;
...
class FileMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('FileMessage render');
        const { message, idx } = this.props;
        const { fileUrl } = message;
...
class LocationMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('LocationMessage render');
        const { message, idx, me, otherGuy, mode } = this.props;
...
class MissedCallMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('MissedCallMessage render');
        const { message, idx, me, otherGuy, mode } = this.props;
...
class PlacesAutocompleteMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('PlacesAutocompleteMessage render');
        const { message, idx, me, otherGuy, mode } = this.props;
...
class OptionsMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('OptionsMessage render');
        const { message, idx, me, otherGuy, mode } = this.props;
...
class BriefJobMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('BriefJobMessage render');
        const { message, idx, me, otherGuy } = this.props;
        const { text, type, job, language } = message;
...
class IDCardMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('IDCardMessage render');
        const { message, idx } = this.props;
...
class NewJoinee extends React.PureComponent {
    ...
    render () {
        ...
        cnsole.info('NewJoinee render: ', idx);
...
class ExcelMessage extends React.PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            modalOpen: false,
    ...
    viewFsFn = () => this.setState({ modalOpen: true, mode: Spreadsheet.MODE_VIEW_FULLSCREEN });
    viewSummaryFn = () => this.setState({ modalOpen: true, mode: Spreadsheet.MODE_VIEW_SUMMARY });
    editFn = () => {
        this.setState({ modalOpen: true, mode: Spreadsheet.MODE_EDITING });
    };
    closeFn = () => {
        this.setState({ modalOpen: false });
    };
    submitFn = ({ title, cells }) => {
        const { onNewMsgFn } = this.props;
        cnsole.info('Excel: ', title, cells);

        const excel = {
            title,
            cells,
        };
        onNewMsgFn({ answer: '', type: OUTPUT_EXCEL, excel });
        this.setState({ modalOpen: false });
    };

    render () {
        cnsole.info('ExcelMessage render: ', this.props.message);
        const { modalOpen, mode } = this.state;
        const { idx, me, message } = this.props;
        const { excel, sender } = message;
        const { title } = excel;

        const imgDim = 25;
        const editAndViewIcons = (
            <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                <View style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    {getCircularImage({ src: EDIT_ICON_BLACK, dim: imgDim, cbFn: this.editFn, border: 0 })}
                </View>
            </View>
        );
        const left  = sender === me.sender ? editAndViewIcons : <View />;
        const right = sender !== me.sender ? editAndViewIcons : <View />;

        const { numRows, numCols } = Spreadsheet.getRowColCount(excel.cells, Spreadsheet.MODE_VIEW_SUMMARY);
        const numValid = Object.values(excel.cells).map(x => x.val).filter(x => x && x.trim() !== '').length;
        const completePercent = numValid / Math.max(1, numRows*numCols);

        return (
            <LongpressWrapper key={idx} {...this.props}>
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: sender === me.sender ? 'flex-end' : 'flex-start', alignItems: 'center' }}>
                    {left}
                    <MessageShell key={idx} {...this.props} message={{...message, type: OUTPUT_EXCEL}}>
                        <View style={{ width: 260, padding: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center' }}>
                            <View style={{ minHeight: 150, maxHeight: 300, width: 250, marginTop: 10, overflow: 'hidden' }}>
                                <TouchableAnim onPress={this.viewFsFn}>
                                    <Spreadsheet {...excel} mode={Spreadsheet.MODE_VIEW_SUMMARY} />
                                </TouchableAnim>
                            </View>
                            <View style={{ width: '80%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                                <View style={{ height: 2, width: 100*completePercent + '%', backgroundColor: '#000000' }} />
                                <View style={{ height: 2, width: 100*(1-completePercent) + '%', backgroundColor: '#c0c0c0' }} />
                            </View>
                            <Text style={{ marginTop: 5, color: '#404040', }}>{title}</Text>
                        </View>
                    </MessageShell>
                    {right}
                    ...
                    {modalOpen && <Modal isOpen={modalOpen} visible={modalOpen} isVisible={modalOpen}
                        ...
                        <View style={{ height: '100%', width: '100%', backgroundColor: 'white',
                            ...
                            <View style={{ height: WINDOW_INNER_HEIGHT, maxHeight: 500, width: WINDOW_INNER_WIDTH, maxWidth: 350, paddingTop: HEIGHT_BUFFER,
                                           display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center' }}>
                                <Spreadsheet {...excel} mode={mode} submitFn={this.submitFn} />
                            </View>
                        </View>
                    </Modal>}
                </View>
...
class TaskListMessage extends React.PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            modalOpen: false,
    ...
    submitFn = ({ title, taskList }) => {
        const { onNewMsgFn } = this.props;
        cnsole.info('taskList: ', title, taskList);

        onNewMsgFn({ answer: '', type: OUTPUT_TASK_LIST, taskList });
        this.closeFn();
    };
    viewFsFn = () => this.setState({ modalOpen: true });

    render () {
        cnsole.info('TaskListMessage render');
        const { modalOpen } = this.state;
        const { idx, me, message } = this.props;
        const { taskList, sender } = message;

        return (
            <LongpressWrapper key={idx} {...this.props}>
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: sender === me.sender ? 'flex-end' : 'flex-start', alignItems: 'center' }}>
                    <MessageShell key={idx} {...this.props} message={{...message, type: OUTPUT_TASK_LIST}}>
                        <View style={{ width: 260, padding: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center' }}>
                            <TouchableAnim onPress={this.viewFsFn}>
                                <View style={{ minHeight: 150, maxHeight: 200, width: 250, marginTop: 10 }}>
                                    <TaskList me={me} sender={sender} payload={taskList} isPreview={true} />
                                </View>
                            </TouchableAnim>
                        </View>
                    </MessageShell>
                    ...
                    {modalOpen && <Modal isOpen={modalOpen} visible={modalOpen} isVisible={modalOpen}
                        ...
                        <View style={{ height: '100%', width: '100%', backgroundColor: 'white',
                                       display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                            <View style={{ height: WINDOW_INNER_HEIGHT, width: WINDOW_INNER_WIDTH, paddingTop: HEIGHT_BUFFER,
                                           display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center' }}>
                                <TaskList me={me} sender={sender} payload={taskList} isPreview={false} closeFn={this.closeFn} submitFn={this.submitFn} />
                            </View>
                        </View>
                    </Modal>}
                </View>
...
class SystemMessage extends React.PureComponent {
    ...
    render () {
        cnsole.info('SystemMessage render');
...
class LikesWidget extends React.PureComponent {
    ...
    render () {
        cnsole.info('LikesWidget render');
        const { id } = this.props;
...
class MessageAPersonModal extends React.PureComponent {
    ...
    render() {
        const { me, sender, modalOpen, idToDetails, ipLocation, closeFn } = this.props;
        const senderName = (idToDetails && sender in idToDetails) ? idToDetails[sender].person.name : sender;
...
const modalStyle = !mobileDetect().isWeb ? {} : {
    content : {
        top                   : '50%',
        left                  : '50%',
        right                 : 'auto',
