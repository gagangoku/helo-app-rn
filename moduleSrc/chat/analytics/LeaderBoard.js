...
export default class LeaderBoard extends React.Component {
    ...
    async componentDidMount() {
        ...
        const detailsObj = {};
        'collection,groupId,user,idx,groupName,groupPhoto,moduleName,members,roleIdToName'.split(',').forEach(x => detailsObj[x] = this.props[x] || getUrlParam(x));

        if (!detailsObj.roleIdToName) {
            const users = lodash.uniq(detailsObj.members.split(','));
            detailsObj.roleIdToName = await getPersonNamesByRoleId(users);
        } else if (typeof detailsObj.roleIdToName === 'string') {
            detailsObj.roleIdToName = JSON.parse(detailsObj.roleIdToName);
        }

        const leaderBoard = await computeLeaderboard(detailsObj);
        this.setState({ leaderBoard, detailsObj });
    }

    render() {
        const { leaderBoard, detailsObj } = this.state;
        if (!leaderBoard) {
            return <View />;
        }

        return <LeaderBoardDisplay {...detailsObj} leaderBoard={leaderBoard} />;
    }
}
...
class LeaderBoardDisplay extends React.Component {
    ...
    render() {
        const { leaderBoard, groupName, groupPhoto, moduleName } = this.props;
...
const getScoresForIdx = (keys, rsp, members) => {
...
const computeLeaderboard = async ({ collection, groupId, user, idx, members, roleIdToName }) => {
    ...
    const scores = getScoresForIdx(keys, rsp, members);
