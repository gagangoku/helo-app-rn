...
export default class GroupAnalytics extends React.Component {
    ...
    async componentDidMount() {
        ...
        const detailsObj = {};
        'collection,groupId,user'.split(',').forEach(x => detailsObj[x] = this.props[x] || getUrlParam(x));
        const { collection, groupId, user } = detailsObj;
        ...
        const analytics = await computeAnalytics({ groupId, messages, members, roleIdToName });
    ...
    render() {
        ...
        return <GroupAnalyticsDisplay analytics={analytics} groupName={groupName} groupPhoto={groupPhoto} />
    }
}

export class GroupAnalyticsNative extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
        };
    ...
    async componentDidMount() {
        const { groupId, groupInfo, roleIdToName } = this.props;
        const { messages, members, photo, name } = groupInfo;
        const analytics = await computeAnalytics({ groupId, messages, members, roleIdToName });
        this.setState({ analytics, groupPhoto: photo, groupName: name });
    }
...
const computeAnalytics = async ({ groupId, messages, members, roleIdToName }) => {
...
class GroupAnalyticsDisplay extends React.PureComponent {
    ...
    render() {
        const { analytics, groupName, groupPhoto } = this.props;
        if (!analytics) {
            return <View />;
        }
